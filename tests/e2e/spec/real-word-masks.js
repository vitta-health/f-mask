...
test('Weight with numeric number input', async (t) => {
  const el = Selector('input#weight-ex');
  await t
    .typeText(el, '1234')
    .expect(el.value).eql('1,234');
});

test('Weight with ten number input', async (t) => {
  const el = Selector('input#weight-ex');
  await t
    .typeText(el, '12345')
    .expect(el.value).eql('12,345');
});


test('Weight with hundred number input', async (t) => {
  const el = Selector('input#weight-ex');
  await t
    .typeText(el, '123456')
    .expect(el.value).eql('123,456');
});

test('Weight with thousand number input', async (t) => {
  const el = Selector('input#weight-ex');
  await t
    .typeText(el, '1234567')
    .expect(el.value).eql('123,456');
